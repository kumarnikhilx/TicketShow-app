  ...
  const createBooking = async () => {
    try {
      ...
      if (!selectedTime || !selectedSeats.length)
        return toast.error("Please select time & seats");

      // 👇 Try to get the user's email from your auth user object
      const userEmail =
      ...
      const { data } = await axios.post(
        "/api/booking/create",
        {
          ...
          selectedSeats,
          email: userEmail, // 👈 send to backend
        },
        {
          headers: {
      ...
      const options = {
        ...
        description: "Movie Ticket Checkout",
        order_id: data.orderId,

        handler: async function (response) {
          const verify = await axios.post("/api/booking/verify", {
            ...response,
